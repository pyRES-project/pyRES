...
class Environmentals:
    """
    Computes environmental performance indicators for a prosumer or REC,
    including CO2 emissions avoided, fossil fuel savings, GWP lifecycle
    assessment and critical raw materials (CRM) inventory.

    The class follows the same pattern as Economics: it receives a list of
    components (systems + BESS) and annual energy flows, then returns a
    dictionary of environmental KPIs.

    :param components: list of System/Bess objects
    :param annual_prod_kwh: float --> total annual production (kWh)
    :param time_horizon: int --> investment time horizon (years)
    """
    ...
    def compute_environmental(self):
        """
        Compute environmental performance indicators.

        :return: env_perf: dict with keys:
            - co2_avoided_annual_t:    float --> tCO2/year avoided
            - co2_avoided_lifetime_t:  float --> tCO2 avoided over time_horizon
            - fuel_savings:            dict  --> per-fuel annual and lifetime savings
            - gwp_embodied_t:          float --> tCO2-eq total embodied emissions
            - gwp_pv_modules_t:        float --> tCO2-eq PV modules
            - gwp_bos_t:               float --> tCO2-eq balance of system
            - gwp_inverter_t:          float --> tCO2-eq inverter (initial + replacements)
            - gwp_bess_t:              float --> tCO2-eq BESS (initial + replacements)
            - gwp_net_t:               float --> tCO2-eq net balance (negative = benefit)
            - co2_payback_years:       float --> years to offset embodied emissions
            - lifecycle_ef:            float --> gCO2-eq/kWh lifecycle emission factor
            - crm_pv:                  dict  --> per-material CRM for PV
            - crm_bess:                dict  --> per-material CRM for BESS
            - crm_total_kg:            float --> total CRM in kg
        """
